// from : https://github.com/keybase/client/blob/master/go/launchd/launchd.go
...
func (s Service) checkPlistPaths(p Plist) error {
	...
	plistDest := s.plistDestination()
	plistDestParent := filepath.Dir(plistDest)
	if _, err := os.Stat(plistDestParent); os.IsNotExist(err) {
		os.Mkdir(plistDestParent, 0644)
	}
...
func (s Service) savePlist(p Plist) error {
	...
	return ioutil.WriteFile(plistDest, []byte(plist), 0644)
}
